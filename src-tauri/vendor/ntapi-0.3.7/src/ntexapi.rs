...
pub unsafe fn NtGetTickCount64() -> ULONGLONG {
    let mut tick_count = ::core::mem::MaybeUninit::<ULARGE_INTEGER>::uninit();
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))] {
        let quad_ptr = tick_count.as_mut_ptr() as *mut ULONGLONG;
        quad_ptr.write(core::ptr::read_volatile(
            core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCountQuad),
        ));
    }
    #[cfg(target_arch = "x86")] {
        loop {
        let s_ptr = (*tick_count.as_mut_ptr()).s_mut();
            (*s_ptr).HighPart =
                read_volatile(&(*USER_SHARED_DATA).u.TickCount.High1Time) as u32;
            (*s_ptr).LowPart = read_volatile(&(*USER_SHARED_DATA).u.TickCount.LowPart);
            if (*s_ptr).HighPart
    ...
    let tick_count = tick_count.assume_init();
...
pub unsafe fn NtGetTickCount() -> ULONG {
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))] {
        ((core::ptr::read_volatile(core::ptr::addr_of!((*USER_SHARED_DATA).u.TickCountQuad))
